"""
Crea vistas unificadas vw_all_* en TiDB.

No modifica tablas actuales, staging ni Streamlit. Si las tablas operativas
actuales no estan en el esquema TiDB activo, el SQL conserva solo la rama
historica y deja la rama actual sin filas.
"""
...
SQL_PATH = PROJECT_ROOT / "sql" / "02_create_unified_views.sql"
...
def existing_tables(engine: Engine, names: list[str]) -> set[str]:
...
def main() -> None:
    ...
    engine = make_engine()
    current_existing = existing_tables(engine, CURRENT_TABLES)
    missing_current = sorted(set(CURRENT_TABLES) - current_existing)
    if missing_current:
        print("Advertencia: estas tablas actuales no estan en el esquema TiDB activo:")
        for table in missing_current:
            print(f"- {table}")
        print("Las vistas vw_all_* se crean con origen_dato='historico' y rama actual vacia.")
    ...
    created = {row.TABLE_NAME for row in rows}
    print("Vistas unificadas creadas/verificadas:")
    for view in EXPECTED_VIEWS:
        status = "OK" if view in created else "NO ENCONTRADA"
        print(f"- {view}: {status}")
